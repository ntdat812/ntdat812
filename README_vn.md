...
[![Open](https://img.shields.io/badge/ch%E1%BB%9D_review-61_pull_request-7d8590?style=flat-square&labelColor=161b22)](https://github.com/search?q=author%3Antdat812+is%3Apr+is%3Aopen&type=pullrequests)
...
Đếm ngày **07/09/2026**, bằng `gh pr list -R <repo> --author ntdat812`, từng repo một. "Đã merge"
nghĩa là pull request đã được merge vào một repo **tôi không sở hữu** — hai mươi bảy trong hai mươi
tám cái nằm trên nhánh mặc định của repo đó, còn cái thứ hai mươi tám, SurfSense #1728, merge vào
`dev`, nhánh mà dự án đó nhận đóng góp, và `dev` chưa về tới `main`. Không tính bất cứ
...
| [Pull request đang mở](https://github.com/search?q=author%3Antdat812+is%3Apr+is%3Aopen&type=pullrequests) | **61** | Đã mở, đang chờ review |
...
MIT, một endpoint đứng trước 350 nhà cung cấp, 62.1k★. Hai mươi hai PR đã merge.
...
13.7k★. Một PR.
**[MODSetter/SurfSense](https://github.com/MODSetter/SurfSense)** — agent nghiên cứu mã nguồn mở,
16.1k★. Một PR, vào nhánh `dev`.

Mười bốn trong hai mươi tám PR đó đóng issue do người khác mở.
[Danh sách đầy đủ](https://github.com/search?q=author%3Antdat812+is%3Apr+is%3Amerged&type=pullrequests).
Mười lăm PR cho thấy phạm vi công việc:

| Pull request | Nội dung |
| --- | --- |
| [SurfSense #1728](https://github.com/MODSetter/SurfSense/pull/1728) `fix(crawler)` | Crawler chỉ kiểm tra đích bằng `validators.url`, thứ phán xét cách viết chứ không phán xét nơi đến, nên endpoint metadata của cloud cùng mọi địa chỉ loopback hay nội bộ đều được fetch từ bên trong mạng của backend. Guard mới resolve host trước và từ chối trừ khi mọi địa chỉ trả về đều định tuyến công cộng — kể cả `::ffff:127.0.0.1`, thứ có `is_loopback` bằng false cho tới khi gỡ mapping, và dải CGNAT mà cách kiểm tra năm-cờ hiển nhiên để lọt. Xử lý [#1709](https://github.com/MODSetter/SurfSense/issues/1709), issue này vẫn đang mở: bản merge nằm trên `dev` nên GitHub không tự đóng nó. |
...
Sáu mươi mốt PR đang mở: hai mươi bảy trên [9router](https://github.com/decolua/9router)
...
[odysseus](https://github.com/odysseus-dev/odysseus) (86.9k★), và một ở
[OpenClaw](https://github.com/openclaw/openclaw) (389k★). Ở OmniRoute, opencodex, Glances và
SurfSense tôi không còn PR nào đang mở. Hai PR ở OmniRoute bị đóng vì trùng với PR người khác mở trước đó
vài tiếng — cùng một bug, cùng một cách sửa, và cả hai lần phần chẩn đoán đều được xác nhận là
đúng trước khi đóng.

Đó là một con số lớn đặt cạnh 28 cái đã merge, và cách đọc trung thực là phần lớn trong đó đang
...
và đọc ra thành xanh: năm mươi tám trong sáu mươi mốt PR xanh ở mọi check mà repo của nó chạy —
...
Ba cái đỏ, và tôi muốn gọi tên chúng ra hơn là làm tròn đi:
